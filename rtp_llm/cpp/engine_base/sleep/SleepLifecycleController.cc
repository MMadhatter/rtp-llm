...
SleepResult SleepLifecycleController::sleep(const SleepOptions& opt) {
    ...
    if (opt.level != 1) {
        return SleepResult::invalidArgument(
            "sleep rejected: unknown level=" + std::to_string(opt.level)
...
SleepResult SleepLifecycleController::wakeUp(const WakeUpOptions& opt) {
    ...
    if (current != SleepState::SLEEPING && current != SleepState::ERROR && current != SleepState::WAKING_UP) {
    ...
    if (!opt.commit_only && ok && hooks_.restoreKvMemoryBackingAndResetMetadata) {
        ...
        ok = invokeHookNoThrow("restoreKvMemoryBackingAndResetMetadata",
                               hooks_.restoreKvMemoryBackingAndResetMetadata);
        if (!ok) {
    ...
    // The old device KV contents were intentionally discarded during sleep,
    // but after wake_up succeeds the device KV memory is backed again and its
...
SleepStatus SleepLifecycleController::status() const {
    ...
    } else if (s.state == SleepState::ERROR && (!s.device_kv_cache_valid || s.kv_memory_state == "PAUSED")) {
        s.gpu_resource_state = "PARTIAL";
    } else {
