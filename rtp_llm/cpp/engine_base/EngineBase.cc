...
void EngineBase::initRuntime(const EngineInitParams& params) {
    ...
    // Startup-fixed sleep level (1 host backup / 2 discard weights / 3 deep-sleep).
    // For 2/3 the weights VMM region was opened without host cpu_backup at load time.
    sleep_controller_.setSleepLevel(params.runtime_config.sleep_mode_level);
    const auto rank =
