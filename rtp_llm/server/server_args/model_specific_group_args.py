...
def init_model_specific_group_args(parser, model_specific_config):
    ...
    model_specific_group.add_argument(
        "--use_engram_gpu_embedding",
        env_name="USE_ENGRAM_GPU_EMBEDDING",
        bind_to=(model_specific_config, "use_engram_gpu_embedding"),
        type=str2bool,
        default=True,
        help="是否使用 Engram GPU embedding。设置为 True 使用 Engram GPU embedding。设置为 使用 CPU embedding。",
    )
