...
def init_engine_group_args(parser, runtime_config):
    ...
    engine_group.add_argument(
        ...
        "流式原地 copy_ 重载（不写磁盘）；3=deep-sleep（cuda-checkpoint）：sleep 前拆除跨进程 "
        "collective（NCCL/DeepEP；symm-mem 在 level-3 下 init 阶段禁用），checkpoint 整个进程使 "
        "GPU→0，唤醒时重建 collective 并像 level-2 重载权重。/sleep 请求的 level 必须与此值一致，"
        "默认 1。仅接受 1/2/3：level 0（state-preserving）尚未实现，非法值在启动期即被 argparse "
        "拒绝，避免 Python 权重层与 C++ RuntimeConfig 对 level 认知分歧",
    )
