...
without spinning up a real DSV4 layer (no DeepGEMM / no real KV pool):
  ...
  * Per-instance cp_gather_stream is cached + reused (FIFO ordering
    contract for layers that issue >1 NCCL collective inside one step,
    e.g. CSA nested-indexer + main compressor).
"""
...
class HCAOverlapStreamCacheTest(unittest.TestCase):
    """Per-instance ``_cp_gather_stream_cached`` reuse — required for
    FIFO ordering across compressor calls in one step (CSA: nested +
    main)."""
