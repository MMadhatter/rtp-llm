...
def forward_layers(
    ...
    **Stage-2 compat shim**: ``Block.forward`` is now flat-native (accepts
    ...
    if cp_ctx is not None:
        ...
        # request's absolute start position (for example cp_rank > 0). Use
        # the CP metadata built from framework context_parallel_info instead
        # of deriving these from rank-local cu_seqlens.
