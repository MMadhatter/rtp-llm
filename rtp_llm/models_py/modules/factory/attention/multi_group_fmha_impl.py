...
def switch_kv_block_ids_to_layer_group(
    attn_inputs: PyAttentionInputs, layer_idx: int
) -> int:
    """Look up the KV cache group for *layer_idx* and switch
    attn_inputs.kv_cache_block_id_{device,host} to that group's tensors.

    Returns the group id (0 when no multi-group mapping exists).
    """
    if not attn_inputs.kv_cache_block_id_device_by_group:
        return 0
    ...
    attn_inputs.kv_cache_block_id_device = (
        attn_inputs.kv_cache_block_id_device_by_group[gid]
    )
    if attn_inputs.kv_cache_block_id_host_by_group:
        attn_inputs.kv_cache_block_id_host = (
            attn_inputs.kv_cache_block_id_host_by_group[gid]
        )
    return gid
...
class MultiGroupFMHAImpl(FMHAImplBase):
    ...
    def forward(self, *args, **kwargs) -> torch.Tensor:
        """Transparent proxy: supports both MHA (qkv, kv_cache) and MLA (q, compressed_kv, k_pe, kv_cache, layer_id) signatures."""
        return self._impls[self._active_gid].forward(*args, **kwargs)
