...
class RoundRobinSparseMlaFp8CPOp(SparseMlaFp8Op):
    """Page-level round-robin CP for Sparse MLA (FP8).

    Block b → rank (b % cp_size). Each rank writes only its owned blocks to the
    sharded kv_cache (via slot_mapping with -1 for non-owned positions).

    For attention there are two paths:
    ...
    def plan(
        ...
        # Page-level RR: all local tokens participate in computation (including
        # padding at tail). Padding tokens compute attention with aliased global
        # positions — harmless since their output is discarded downstream.
        self.total_local_ids = torch.arange(
            local_tokens, device=self.device, dtype=torch.long
        )

        # Map local → padded global → unpadded global.
        # Padding positions alias to the last valid token's unpadded index via
        # pad_to_unpad (cumsum-based), safe for batch_indice_d/positions_d access.
        restore = kv_restore_indices.to(device=self.device, dtype=torch.long)
        inv_restore = torch.empty(restore.size(0), device=self.device, dtype=torch.long)
        inv_restore[restore] = torch.arange(
            restore.size(0), device=self.device, dtype=torch.long
        )
        source_flat = self.prefill_cp_rank * local_tokens + self.total_local_ids
        ...
        # Valid mask: padding tokens must not write to cache (slot_mapping = -1)
        self._local_valid_mask = padding_mask_d[total_global_ids_padded] == 1
        ...
        if total_ws_tokens > 0:
            ...
            req_ids = torch.repeat_interleave(
                torch.arange(lengths.size(0), dtype=torch.long), lengths
            )
        ...
        # Page-level RR: each rank owns blocks where block_idx % cp_size == cp_rank.
        # All ranks must use the same local_blocks count = ceil(total_blocks / cp_size)
        # to ensure all_gather tensor shapes match across ranks.  Ranks that own
        # fewer real blocks simply have padding entries (zeros) in their local shard.
        # This matches C++ CPSlotMapper::localBlockCount.
        total_blocks_per_req = (kv_lengths_t + page_size - 1) // page_size
        local_blocks_per_req = (total_blocks_per_req + C - 1) // C
        local_kv_counts_t = local_blocks_per_req * page_size
        ...
        if total_kv > 0:
            kv_lengths_long = kv_lengths_t.long()
            positions_flat = torch.cat(
                [torch.arange(kv_len, dtype=torch.long) for kv_len in kv_lengths_list]
            )
            req_ids = torch.repeat_interleave(
                torch.arange(kv_lengths_long.size(0), dtype=torch.long), kv_lengths_long
            )
            cu_offsets = cu_local_kv_cpu[req_ids].long()
            global_block_idx = positions_flat // page_size
            token_in_block = positions_flat % page_size
            ranks = global_block_idx % C
            local_block_idx = global_block_idx // C
            local_idx = local_block_idx * page_size + token_in_block
        ...
        if total_kv > 0:
            ...
            vblock_idx = global_block_idx // C
            kernel_block_off = token_in_block // kernel_page_size
            kernel_block_idx_d = (vblock_idx * K + kernel_block_off).to(self.device)
            token_in_kernel_d = (token_in_block % kernel_page_size).to(self.device)
            physical_slots = (
        ...
        # Pre-index per-token fmha params once (reused by _run_cp_logits_topk every layer)
        if mla_params.ks is not None and self.total_global_ids.numel() > 0:
            self._cp_ks = mla_params.ks[self.total_global_ids]
            self._cp_ke = mla_params.ke[self.total_global_ids]
            self._cp_expanded_seq_lens = mla_params.expanded_seq_lens[
                self.total_global_ids
            ]
            self._cp_topk_indices_offset = mla_params.topk_indices_offset[
                self.total_global_ids
            ]
        else:
            self._cp_ks = None
            self._cp_ke = None
            self._cp_expanded_seq_lens = None
            self._cp_topk_indices_offset = None
    ...
    def _build_local_slot_mapping(
        ...
        For owned tokens, the global slot_mapping has the correct physical slot.
        For non-owned tokens, slot_mapping has -1.
        Padding tokens (identified by _local_valid_mask) are forced to -1 so they
        are not written to cache.
        ...
        result = global_slot[self.total_global_ids].clone()
        # Padding tokens alias to valid positions via pad_to_unpad — mask them out
        result[~self._local_valid_mask] = -1
        return result

    def _should_use_prefix_q_path(self) -> bool:
        """Return whether prefix-hit should use the q-path (all-gather Q)
        instead of the workspace path (all-gather KV).

        Compares communication cost:
        - KV path: (kv_lora_rank + qk_rope_head_dim) * 2 bytes/token (BF16)
        - Q path:  num_heads * (qk_head_dim * 2 + kv_lora_rank * 2 + 4) + top_k * 4 bytes/token
                   (q BF16 + out BF16 + lse FP32 + topk INT32)

        Q-path cheaper when: q_len * q_cost < kv_len * kv_cost
        i.e. q_len / kv_len < kv_cost / q_cost
        """
        ...
        if global_q_len <= 0 or total_kv_len <= 0:
            return True
        kv_bytes_per_token = (self.kv_lora_rank + self.qk_rope_head_dim) * 2
        q_bytes_per_token = (
            self.num_heads * (self.qk_head_dim * 2 + self.kv_lora_rank * 2 + 4)
            + self.top_k * 4
        )
        threshold = kv_bytes_per_token / q_bytes_per_token
        return (global_q_len / total_kv_len) <= threshold
    ...
    def _forward_prefix_cache_ag_q(
        ...
        # Padding tokens already included (topk has local_tokens rows).
        # For no_valid case, create dummy -1 topk for the collective.
        if no_valid:
            topk_for_gather = torch.full(
        ...
        # Filter to unpadded global tokens for attention
        ...
        # Select this rank's tokens from merged global output.
        # Padding positions alias to valid tokens — harmless, discarded downstream.
        return torch.index_select(merged_out, 0, self.total_global_ids)
...
class SparseMlaCpImpl(SparseMlaImpl):
    ...
    def prepare(self, attn_inputs: PyAttentionInputs, forbid_realloc: bool = False):
        ...
        self.cp_params = SimpleNamespace(
            ...
            # Pre-indexed fmha params for _run_cp_logits_topk (avoids per-layer indexing)
            cp_ks=getattr(impl, "_cp_ks", None),
            cp_ke=getattr(impl, "_cp_ke", None),
            cp_expanded_seq_lens=getattr(impl, "_cp_expanded_seq_lens", None),
            cp_topk_indices_offset=getattr(impl, "_cp_topk_indices_offset", None),
        )
    ...
    def forward(
        ...
        if self.fmha_impl.total_local_ids.size(0) > 0:
            ...
            if self.fmha_impl.kv_cache_sharded:
