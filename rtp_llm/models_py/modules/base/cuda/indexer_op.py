...
class IndexerOp(nn.Module):
    ...
    def quant_q_k_cp(
        ...
        Side effect:
            Sets self.kv_len from kv_restore_unpad_indices for CP topk methods to use
    ...
    def _run_cp_logits_topk(
        ...
        Selects this rank's q tokens via total_local_ids / total_global_ids,
        runs fp8_mqa_logits against full KV, then ragged topk.

        Args:
            q_fp8: Local quantized query [local_tokens, index_n_heads, index_head_dim].
            weights: Weights [local_tokens, index_n_heads, 1].
            kv_fp8: (k_fp8, k_scale) tuple covering all global KV tokens.
            fmha_params: Must have ks, ke, expanded_seq_lens, topk_indices_offset.

        Returns:
            TopK indices for this rank's tokens, or None if no tokens.
        """
        from rtp_llm.models_py.kernels.cuda.fast_topk import (
            fast_topk_transform_ragged_fused,
        )
    ...
    # ---- CP topk: round-robin (sharded cache + workspace) ----
