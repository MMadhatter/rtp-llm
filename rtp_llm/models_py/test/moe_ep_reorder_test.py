...
class MoeReorderTest(TestCase):
    ...
    def _run_moe_ep_reorder_test(
        ...
        print(
            f"testing >>>>> num_tokens:{num_tokens}, num_expert:{num_expert}, hidden_dim: {hidden_dim}, top_k:{top_k}, ep_size: {ep_size}>>>>>>"
        )
        ...
        torch.equal(src_row_id2dst_row_id_map, inv_permuted_idx)
        torch.equal(torch_permuted_hidden_states, permuted_hidden_states)
        ...
        cuda_out = torch.empty_like(permuted_hidden_states)
        moe_unpermute(
            out=cuda_out,
            permuted_hidden_states=torch_permuted_hidden_states,
            topk_weights=topk_weights,
            inv_permuted_idx=inv_permuted_idx,
            expert_first_token_offset=expert_first_token_offset,
        )
        torch.equal(torch_out, cuda_out)
