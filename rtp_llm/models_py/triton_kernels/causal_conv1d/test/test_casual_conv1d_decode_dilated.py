...
from rtp_llm.models_py.triton_kernels.causal_conv1d import causal_conv1d_update_dilated
...
def causal_conv1d_update_ref(
    ...
    out = F.conv1d(
        x_new, weight.unsqueeze(1), bias, padding=0, dilation=dilation, groups=dim
    )[:, :, -seqlen:]
    if unsqueeze:
        out = out.squeeze(-1)
...
class TestCausalConv1dUpdateWithDilation(unittest.TestCase):
    def test_causal_conv1d_update_dilated_no_spec(self):
        ...
        assert seq_size_per_block >= (width - 1) * dilation
        ...
        for batch in [1, 2, 10, 16, 32]:
            for seqlen in [
                1
            ]:  # speculatve decoding is not supported for dilation > 1 now
                print(f"test_causal_conv1d_update_with_dilation batch {batch}")
                assert seqlen == 1
                ...
                activation = "silu"
                cache_seqlens = None
                out = causal_conv1d_update_dilated(
                    x,
                    conv_state,
                    weight,
                    bias,
