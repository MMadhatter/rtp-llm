...
from rtp_llm.models_py.triton_kernels.causal_conv1d import causal_conv1d_fn_dilated
...
def causal_conv1d_ref(
    ...
    dilation: int,
    bias: (dim,)
    initial_states: (batch, dim, (width - 1) * dilation)
    final_states_out: (batch, dim, (width - 1) * dilation)
    ...
    if initial_states is None:
        ...
        out = F.conv1d(
            x, weight.unsqueeze(1), bias, padding=0, groups=dim, dilation=dilation
        )
    out = out[..., :seqlen]
    if return_final_states:
        final_states = F.pad(x, ((width - 1) * dilation - x.shape[-1], 0)).to(
            dtype_in
        )  # (batch, dim, (width - 1) * dilation)
...
class TestCausalConv1dPrefillWithDilation(unittest.TestCase):
    # without KVCache
    def test_basic(self):
        device = "cuda"
        ...
        for batch in [1, 2, 10]:
            ...
            for b in range(batch):
                ...
                out_ref.append(
                    causal_conv1d_ref(
                        x_s, weight_ref, dilation, bias_ref, activation=activation
                    )
                )
    ...
    def test_prefill(self):
        ...
        for batch in [1, 4, 16]:
            ...
            assert (conv_kernel_size - 1) * dilation <= seq_size_per_block

            block_num = [
                math.ceil(length / seq_size_per_block) for length in input_length
            ]
            conv_states = torch.zeros(
                [sum(block_num), (conv_kernel_size - 1) * dilation, dim],
                device=device,
                dtype=torch.float32,
            )
            ...
            for b in range(batch):
                for i in range(1, block_num[b] + 1):
                    if i != block_num[b] or input_length[b] % seq_size_per_block == 0:
                        continue
                    end_idx = (
                    ...
                    start_idx = end_idx - ((conv_kernel_size - 1) * dilation)
                    try:
    ...
    def test_prefill_with_initial_state(self):
        ...
        for batch in [1, 2, 4, 10]:
            ...
            # batch = 1
            origin_length = [
                random.randint(seq_size_per_block, 4096) for _ in range(batch)
            ]
            ...
            for i in range(batch):
                ...
                for j in range(reuse_block_num[i]):
                    conv_states[block_offset + j] = x[
                        ...
                        - ((conv_kernel_size - 1) * dilation) : offset
                        + (j + 1) * seq_size_per_block
                    ]
