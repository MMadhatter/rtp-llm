...
class BaseModel(object):
    ...
    def _cleanup_loader_resources(self):
        ...
        例外：level-2/3 sleep（丢弃权重）在 wake 时需要由 model loader 从原始
        ...
        "ts is empty" 失败。因此 level-2/3 保留 database；其余模式照旧释放。保留的
        ...
        if is_enabled() and sleep_mode_level() in (2, 3):
            logging.info(
                "sleep level-2/3: keeping loader database alive for wake-time "
